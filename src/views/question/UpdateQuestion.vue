        ...
        <a-form :model="questionForm">
            ...
            <a-form-item label="测试用例配置">
                <a-space direction="vertical" fill>
                    <a-form-item
                        v-for="(judgeCases, index) of questionForm.judgeCases"
                        :key="index"
                        no-style
                    >
                        <a-space direction="horizontal" fill>
                            <div style="display: flex">
                                <a-form-item
                                    :field="`judgeCasess[${index}].input`"
                                    :label="`输入用例-${index}`"
                                    :key="index"
                                    style="min-width: 500px"
                                >
                                    <a-input
                                        v-model="judgeCases.input"
                                        placeholder="请输入测试输入用例"
                                    />
                                </a-form-item>

                                <a-form-item
                                    :field="`judgeCasess[${index}].output`"
                                    :label="`输出用例-${index}`"
                                    :key="index"
                                    style="min-width: 500px"
                                >
                                    <a-input
                                        v-model="judgeCases.output"
                                        placeholder="请输入测试输出用例"
                                    />
                                </a-form-item>

                                <a-button
                                    @click="deljudgeCases(index)"
                                    status="danger"
                    ...
                    <a-form-item style="margin-left: -120px">
                        <a-button type="outline" status="success" @click="addjudgeCases"
                            >新增测试用例
                        </a-button>
                    </a-form-item>
                </a-space>
...
const loadQuestionById = async (questionId: number) => {
    const res = await Service.getQuestionByIdUsingGet(questionId)
    if (res.code === 0) {
        Object.assign(questionForm, res.data)
        const judgeCase = res.data?.judgeCase
        if (judgeCase) {
            questionForm.judgeCases = JSON.parse(judgeCase)
        }
        const judgeConfig = res.data?.judgeConfig
        if (judgeConfig) {
            questionForm.judgeConfig = JSON.parse(judgeConfig)
        }
        const tags = res.data?.tags
        if (tags) {
            questionForm.tags = JSON.parse(tags)
        }
    } else {
...
onMounted(async () => {
    await loadQuestionById(route.query.id as any)
})
 ...
 * @param index
 */
const deljudgeCases = (index: number) => {
...
const doSubmit = async () => {
    const res = await Service.updateQuestionUsingPost(questionForm)
    if (res.code === 0) {
        router.push({
